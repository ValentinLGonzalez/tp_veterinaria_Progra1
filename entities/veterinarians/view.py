...
from entities.veterinarians.entity import READABLE_HEADER
from utils.constants import HEADER_VETERINARIAN

def modify_veterinarian(id):
...
def load_dynamic_table(container):
    ...
    for tuple_row in data:
        ...
        tk.Button(btn_frame, text="Eliminar", bg="#ffcccc", 
                  command=lambda i=row_id: delete_veterinarian(i)).pack(side="left", padx=2)
        tk.Button(btn_frame, text="Editar", 
                  command=lambda i=row_id: modify_veterinarian(i)).pack(side="left", padx=2)
