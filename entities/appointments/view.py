...
from entities.appointments.entity import READABLE_HEADER
...
def load_dynamic_table(container):
    ...
    data = show_all_appointments_action() 
    for tuple_row in data:
        ...
        for index in index_to_show:
            valor = tuple_row[index+1]
            tk.Label(row_frame, text=str(valor), width=20, anchor="w").pack(side="left", padx=5)

        btn_frame = tk.Frame(row_frame)
        btn_frame.pack(side="right", padx=5)
        
        tk.Button(btn_frame, text="Eliminar", bg="#ffcccc", 
                  command=lambda i=row_id: delete_appointment(i)).pack(side="left", padx=2)
        tk.Button(btn_frame, text="Editar", 
                  command=lambda i=row_id: modify_appointment(i)).pack(side="left", padx=2)
