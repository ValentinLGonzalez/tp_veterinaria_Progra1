...
def delete_pet_by_id(pet_id, array_pets):
    current_pets_id = [pet[ENCAB_MASCOTAS.index("mascota_id")] for pet in array_pets]
    if pet_id in current_pets_id:
        deleted_pet_index = current_pets_id.index(pet_id)
        array_pets[deleted_pet_index][ENCAB_MASCOTAS.index("activo")] = False

def show_pet(pet):
    print()
    print("Mascota agregada/modificada correctamente.")
    print()
    print_array(ENCAB_MASCOTAS, pet)

def get_pet_by_id(pet_id, array_pets):
    for pet in array_pets:
        if pet[ENCAB_MASCOTAS.index("mascota_id")] == pet_id:
            return pet
    return None
...
def modify_pet_action(pets):
    ...
    if pet_to_update:
        ...
        return update_pet_by_id(updated_pet[ENCAB_MASCOTAS.index("mascota_id")], updated_pet, pets)
    else:
        print("Mascota no encontrada.")
        return None

def show_all_pets_action(array_pets):
    active_pets = list(filter(lambda p: p[ENCAB_MASCOTAS.index("activo")] == True, array_pets))
    print_array_bidimensional(ENCAB_MASCOTAS, active_pets)
...
def delete_pet_action(pets):
    ...
    if pet_to_delete:
        ...
        delete_pet_by_id(pet_to_delete[ENCAB_MASCOTAS.index("mascota_id")], pets)
    else:
        print("Mascota no encontrada.")
